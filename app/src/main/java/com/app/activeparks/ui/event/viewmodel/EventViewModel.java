...
public class EventViewModel extends ViewModel {
    ...
    public void applyUser() {
        if (Objects.requireNonNull(mItemEvent.getValue()).getEventUser() == null) {
            Disposable joinEvent = repository.jointEvent(mId).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(result -> getEvent(mId),
                    error -> {
                    });

            compositeDisposable.add(joinEvent);
        } else {
    ...
    public void location(double lat, double lon) {
        Disposable repositoryLocation =   repository.location("" + lat, "" + lon).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
                .subscribe(result -> {
