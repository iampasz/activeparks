...
import com.technodreams.activeparks.databinding.FragmentSaveRouteBinding
...
class SaveRouteActiveFragment : Fragment() {
    ...
    private lateinit var binding: FragmentSaveRouteBinding
    ...
    private val viewModel: SaveRouteActiveViewModel by viewModel()
    ...
    companion object {
        fun show(routeActiveId: String): SaveRouteActiveFragment {
            val fragment = SaveRouteActiveFragment()
            val args = Bundle()
            args.putString("route_active_id", routeActiveId)
            fragment.arguments = args
            return fragment
        }

        fun insert(routeActiveId: String): SaveRouteActiveFragment {
            val fragment = SaveRouteActiveFragment()
            val args = Bundle()
            args.putString("insert_route_active_id", routeActiveId)
            fragment.arguments = args
            return fragment
        }
    }
    override fun onCreateView(
        ...
        binding = FragmentSaveRouteBinding.inflate(inflater, container, false)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding) {
            arguments?.getString("insert_route_active_id")?.let {
                viewModel.insert(it)
            }
            arguments?.getString("route_active_id")?.let {
                viewModel.getRouteActive(it)
            }
    ...
    private fun FragmentSaveRouteBinding.setListener() {
        ...
        tvDeleteTrack.setOnClickListener {
            viewModel.removeRouteActive()
        }

        bSaveTrack.setOnClickListener {
            viewModel.saveRouteActive()
        ...
        with(viewModel) {
            trackDate.observe(viewLifecycleOwner) { response ->
                ...
                response.coverType.let {coverTypeId ->
                    if (coverTypeId?.isNotEmpty() == true) {
